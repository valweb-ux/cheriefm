...
export default async function ArtistPage({ params }: ArtistPageProps) {
  ...
  const artistData = artist || {
    id: params.artistId,
    name: "Виконавець " + params.artistId,
    image_url: "/placeholder.svg?height=300&width=300",
    genre: "Поп",
    bio: "Біографія виконавця " + params.artistId,
  }
  ...
  // Покращена обробка поля bio
  const artistBio = (() => {
    if (!artistData.bio) return null

    if (typeof artistData.bio === "string") {
      return artistData.bio
    }

    if (typeof artistData.bio === "object") {
      if (artistData.bio.text) return artistData.bio.text
      // Якщо є інші поля в об'єкті bio, можна їх обробити тут
    }
